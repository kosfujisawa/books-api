...
export class AuthController {
    ...
    async confirmSignUp(@Body() confirmSignUpDto: ConfirmSignUpDto): Promise<ConfirmSignUpCommandOutput> {
        return await this.cognitoService.confirmSignUp(confirmSignUpDto);
    }
    
    @Post('login')
    async signIn(@Body() signInDto: SignInDto): Promise<InitiateAuthCommandOutput> {
